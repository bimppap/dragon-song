...
def ensure_schema(engine: Engine) -> None:
    ...
    if "is_public" not in character_columns:
    ...
    if "items" in table_names:
        ...
        if "image_after_purchase_url" not in item_columns:
            statements.append("ALTER TABLE items ADD COLUMN image_after_purchase_url VARCHAR")
        ...
        if "sort_order" not in item_columns:
            ...
            statements.append("UPDATE items SET sort_order = id")
    ...
    if "settlement_requests" in table_names:
        ...
        if "target_character_ids" not in settlement_columns:
            statements.append(
                "ALTER TABLE settlement_requests ADD COLUMN target_character_ids JSON NOT NULL DEFAULT '[]'"
            )
    ...
    if "skill_nodes" in table_names:
        if "tier6_effect" not in {col["name"] for col in inspector.get_columns("skill_nodes")}:
            statements.append("ALTER TABLE skill_nodes ADD COLUMN tier6_effect VARCHAR")
