...
/** 편집 시 기존 캐릭터 값을 폼에 채운다. handleSubmit의 변환식을 그대로 되돌린다(비율 스탯은 값×100 방식, 0이면 0%). */
function toFormFromCharacter(character: Character): CharacterCreateForm {
  const rawPct = (v: number) => String(Math.round(v * 100));
  return {
    name: character.name,
    hp: String(character.hp),
    hp_max: String(character.hp_max),
    hp_max_p: rawPct(character.hp_max_p),
    hp_regen_true: String(character.hp_regen_true),
    hp_regen_fixed: rawPct(character.hp_regen_fixed),
    mp: String(character.mp),
    mp_max: String(character.mp_max),
    mp_regen: String(character.mp_regen),
    atk: String(character.atk),
    atk_p: rawPct(character.atk_p),
    def: String(character.def),
    def_p: rawPct(character.def_p),
    def_eff: rawPct(character.def_eff),
    attn: String(character.attn),
    presence: rawPct(character.presence),
    heal_eff: rawPct(character.heal_eff),
    sh: String(character.sh),
    dmg_p: rawPct(character.dmg_p),
    dmg_r: rawPct(character.dmg_r),
    skill_lv: String(character.skill_lv),
    skill_eff_true: String(character.skill_eff_true),
    skill_eff_fixed: rawPct(character.skill_eff_fixed),
...
export default function CharacterCreate({ onCreated, character, onSaved, onCancel }: Props) {
  ...
  function handleFactionChange(value: string) {
    const next = value === "none" ? null : value as Faction;
    setFaction(next);
    // 신규 생성 시에만 포지션 기본 피해 감소값(수비 50%, 그 외 30%)을 자동으로 채워 준다.
    if (!isEditing) {
      setForm((prev) => ({ ...prev, dmg_r: next === "수비" ? "50" : "30" }));
    }
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const payload = {
        ...
        hp_max_p: Number(form.hp_max_p) / 100,
        hp_regen_true: Number(form.hp_regen_true),
        hp_regen_fixed: Number(form.hp_regen_fixed) / 100,
        mp: Number(form.mp),
        mp_max: Number(form.mp_max),
        mp_regen: Number(form.mp_regen),
        atk: Number(form.atk),
        atk_p: Number(form.atk_p) / 100,
        def: Number(form.def),
        def_p: Number(form.def_p) / 100,
        def_eff: Number(form.def_eff) / 100,
        attn: Number(form.attn),
        presence: Number(form.presence) / 100,
        heal_eff: Number(form.heal_eff) / 100,
        sh: Number(form.sh),
        dmg_p: Number(form.dmg_p) / 100,
        dmg_r: Number(form.dmg_r) / 100,
        skill_lv: Number(form.skill_lv),
        skill_eff_true: Number(form.skill_eff_true),
        skill_eff_fixed: Number(form.skill_eff_fixed) / 100,
