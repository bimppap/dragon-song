    ...
    <header className="sticky top-0 z-50 w-full border-b border-slate-200 bg-white shadow-sm">
      <div className="mx-auto flex h-14 max-w-6xl items-center px-4 sm:px-6">
          ...
          {navItems.map(({ href, label, icon: Icon }) => (
            <Link
              ...
              className={cn(
                ...
                isActive(href) ? "bg-indigo-600 text-white" : "text-slate-600 hover:bg-slate-100 hover:text-slate-900",
              )}
            >
          ...
          {member ? (
            <>
              <span className="text-[11px] font-semibold text-slate-500">
                {member.login_id}
                <span className="ml-1.5 rounded-full bg-slate-100 px-2 py-0.5 text-[10px] text-slate-500">{member.role}</span>
              </span>
        ...
        <button
          ...
          className="ml-auto rounded-lg p-2 text-slate-600 transition-colors hover:bg-slate-100 md:hidden"
          aria-label="메뉴"
