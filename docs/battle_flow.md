...
> 현재 사용하지 않아 전투 편성 화면에서 **페어 전투** 선택지를 감춰 두었습니다(`apps/web/lib/battlePairs.ts`의 `PAIR_BATTLE_ENABLED`). 기능 코드는 그대로 남아 있어 값을 `true`로 바꾸면 다시 쓸 수 있습니다.

캐릭터 선택에서 **페어 전투**를 체크하면 선택한 인원이 무작위로 2인 1조에 배정됩니다. **다시 랜덤 매칭**으로 전체를 재배정하거나, 이동 손잡이를 다른 페어의 캐릭터에 드롭해 두 캐릭터의 자리를 교환할 수 있습니다. 손잡이를 차례로 누르는 방식도 지원합니다.

- 홀수 인원일 때는 한 명이 **페어 대기**로 표시되며 전투에는 정상 참여합니다. 인원 추가/제외 시 완성된 페어를 유지하고 남은 인원만 연결합니다.
- `BattleSession.pair_battle`과 `pairs`에 편성을 저장합니다. `pairs`는 캐릭터 ID의 2인 배열 목록이며 홀수일 때만 1인 배열을 하나 허용합니다. 전투 생성 요청에서 편성을 생략하면 서버가 무작위로 배정합니다.
- 전투 중에는 관리자/스텝이 `PUT /battles/{session_id}/pairs`로 변경할 수 있습니다. 관전과 종료된 전투에서는 편성을 표시만 하며, 변경된 편성은 실시간으로 공유합니다. 페어 상대가 바뀌면 포지션과 기술도 바뀌므로 기존 행동/대상 초안을 초기화하고 다시 선택합니다.
- 턴 되돌리기는 현재 편성을 유지하되, 되돌린 참가자 목록에 맞춰 빠진 캐릭터와 대기 인원을 정리합니다.
- 페어끼리 **포지션, HP/MP, 용기·인내·자애·지혜, 성장 등급 및 상세 전투 능력치, 기술**을 교환합니다. 원본 캐릭터와 기술 소유권은 변경하지 않습니다. 이름·초상화·아이템·행동 주체와 보상 귀속은 본인을 유지합니다. 페어 대기 캐릭터는 본인 능력치/기술을 사용합니다.
- 전투 참가 시점의 본래 능력치는 참가자 JSON의 `_pair_original_stats`에 보관하고, 현재 빌린 기본 능력치는 `_pair_base_stats`에 보관합니다. 두 값은 공개 응답에서 제외합니다. `pair_source_character_id`/`pair_source_name`은 실제 적용 중인 능력치와 기술의 원래 소유자를 가리킵니다. 기술 목록 및 실제 발동은 이 소유자의 기술 습득/사용자 지정 이름·이미지를 기준으로 계산하며, 아이템 목록과 소모량은 행동한 본인을 기준으로 계산합니다.
- 재매칭/난입/되돌리기에서는 상대가 현재 빌린 값이 아닌 **상대의 본래 능력치**를 적용합니다. 현재 HP·MP 비율과 본인에게 쌓인 주목도·환경·상태 효과·남은 보호막은 유지합니다. 능력치에 가산된 아이템/상태 효과 수치도 유지합니다.
- 실전 종료(승리/패배/조기 종료) 시 `전투 잔여 HP / 전투 최대 HP × 본래 캐릭터.hp_max`를 반영합니다. 소수점은 내리되 생존한 캐릭터는 최소 1 HP, 기절한 캐릭터는 0 HP이며 본래 최대 HP를 넘지 않습니다. 예를 들어 상대 능력치로 `100/1000 HP`가 남으면 본래 최대 HP가 200인 캐릭터는 20 HP로 돌아옵니다. **MP는 기존 규칙대로 본래 최대 MP까지 회복**합니다. 모의전 결과는 원본에 반영하지 않습니다.
...
3. 관리자가 에너미별로 고른 행동(무반응 / 지정 공격 / 광역 공격 / 소환)을 검증합니다.
   ...
   - 기술에 피격 디버프 · 방어 무시 피해가 있으면 로그와 에너미 카드에 `예상 피해 10 + 방어 무시 7`처럼 따로 붙여 표시합니다(4-5 참고).
...
- HP가 0이 되면 기절 처리되며, 해당 참가자의 전투 주목도는 0으로 초기화되고 보유한 환경·상태 효과 스택은 모두 제거됩니다. 이 시점에 참가자 전원이 기절/퇴각 상태가 되면 `status = "defeat"`로 즉시 전투가 종료됩니다(2턴·3턴을 기다리지 않음).
...
| 0.5 | **반격 태세 만료 처리** — 이전 라운드에 걸린 `반격` 상태는 여기서 제거된다(자기 라운드에만 유지). |
| 1 | **방어 태세 표시** — 모든 참가자의 `defending`/`protect_target`을 초기화한 뒤, `방어`를 선택한 캐릭터만 다시 표시(실제 경감은 3턴에서 적용). |
| 2 | **캐릭터 행동 처리** — `공격/기술 사용`, `아이템 사용`, `구조`, `퇴각`, `치유`를 모두 하나의 큐로 모아 **발동 순서**(4-0절 참고)대로 처리한다. `치유`는 처리되는 시점에 에너미가 이미 전부 죽어 있으면(그 사이 다른 행동으로 승리가 확정됐다면) 생략된다. |
| 3 | 전원 기절/퇴각(전멸)했는지, 에너미가 전멸했는지 확인. 승리도 전멸도 아니면 `phase`를 `"enemy"`로 바꿉니다(round는 아직 증가하지 않음). |

같은 발동 순서 안에서는 프론트에서 보낸 캐릭터 배열 순서대로 처리됩니다. 에너미/소환수 행동은 이 턴에서 처리하지 않습니다(3턴에서 처리).
...
| 2 | **소환수 행동 처리** — 암시 턴에 새로 소환된 하수인을 포함해 살아있는 공격형 소환수가 자동으로 한 명을 공격. |
...
아군 턴의 `공격/기술 사용`, `방어`, `치유`, `구조`, `아이템 사용`, `퇴각` 행동은 제출된 순서가 아니라 **발동 순서**(숫자가 낮을수록 먼저 개시)대로 정렬한 뒤 하나의 큐로 처리됩니다(`resolve_battle_ally_turn`, `BATTLE_ACTION_KIND_PRIORITY`). 기술이 아닌 행동의 발동 순서는 고정값이고, 기술 사용은 그 기술 노드에 저장된 `activation_order` 값을 그대로 씁니다(값이 없으면 공격과 동일한 8로 취급).
...
| 방어 | 3 |
| 치유 | 5 |
| 공격 / 기술 사용(`activation_order` 미지정 시) | 8 |
| 무반응 | 9(실제로는 아무 처리도 하지 않으므로 큐에 들어가지 않음) |

같은 발동 순서를 가진 행동끼리는 프론트에서 보낸 캐릭터 배열 순서(제출 순서)대로 처리됩니다.

### 공통: 마나 소모 / 기술등급계수

마나는 **"기술 사용"(`kind === "skill"`)일 때만** 소비됩니다. 그 외 행동(`공격`, `방어`, `치유`, `구조`, `아이템 사용`, `퇴각`)은 마나를 전혀 쓰지 않습니다. 기술 사용 시 필요한 마나는 그 캐릭터의 `기술 비용(skill_cost)`이며, 발동 순서가 됐을 때 마나가 부족하면(`mp < skill_cost`) **그 기술 사용 자체가 취소**됩니다(경고 로그만 남고 아무 효과도 발생하지 않음 — 과거의 "위력에는 영향 없이 소모만 생략" 규칙은 폐지). 마나가 충분하면 그만큼 소모하고 정상적으로 발동합니다. "공격"과 "기술 사용"은 피해 계산식 자체는 동일하고, 마나 소모 여부만 다릅니다.
...
즉시 피해를 주지 않고, 이번 라운드에 피격 시 아래처럼 피해를 경감합니다(4-4, 5-2, 5-3, 6 참고). 포지션별로 고정된 별도 감소율은 없으며, 캐릭터 본인의 `피해 감소` 스탯(기본값: 공격/치유 30%, 수비 50%)이 그대로 적용됩니다.

```
방어경감량 = 내림(방어력 × (1 + 방어력 증폭) × (1 + 방어 효율))
최종 피해 = max(0, 기본 피해 − 방어경감량)
```
...
3) 이번 라운드 '방어'를 선택했다면: dmg = max(0, dmg − 방어경감량)
...
- 공격 기술에 **피격 디버프 · 방어 무시 피해**(`on_hit_effect = "true_damage"`)가 설정돼 있으면, 1~5의 기술 피해와 별개로 실제로 맞은 캐릭터(보호로 대신 맞은 방어자 포함)에게 `dot_damage`만큼 고정 피해를 한 번 더 줍니다. 방어력·피해 감소(반격/경호 감소 포함)와 에너미의 피해 증감은 적용하지 않고, 보호막만 먼저 흡수합니다. 상태 효과로 남지 않으므로 디버프 해제 대상이 아닙니다. (다른 피격 디버프인 "턴마다 고정 피해"는 다음 암시 턴부터 매 턴 HP를 깎고, "상세 능력치 변경"은 해제될 때까지 능력치를 바꿉니다.)
...
에너미 관리에서 **행동횟수(`action_count`)**를 설정합니다. 기본값은 1이고 **1 이상의 정수**만 저장할 수 있습니다. 기존 에너미는 자동으로 1회로 초기화됩니다. 전투 시작/난입 시 횟수를 스냅샷에 저장하므로 관리 설정을 바꾸어도 진행 중 전투의 횟수는 바뀌지 않습니다.

- 암시 턴에는 행동 가능한 각 에너미의 횟수만큼 스킬을 골라야 합니다. 같은 스킬을 여러 번 선택할 수 있고, 행동별 대상도 따로 지정합니다. 스킬이 없는 에너미만 무반응을 선택합니다.
- 화면에 `1번째 행동`, `2번째 행동` 순서로 표시하며 **앞으로/뒤로** 버튼은 스킬과 대상을 함께 이동합니다. 에너미 카드 순서, 각 카드의 행동 순서대로 확정합니다.
- 서버는 누락·초과·유효하지 않은 스킬을 거부합니다. 새 설정 필드가 없는 이전 버전의 진행 중 전투만 기존의 생략된 1회 행동을 허용합니다.
- `pending_enemy_actions`는 같은 에너미 ID가 여러 번 들어가는 **순서 있는 배열**입니다. 확정·저장·재접속·관전·턴 되돌리기에서 모든 행동과 순서를 유지합니다. 운영진의 암시 편집 초안도 행동 배열 전체를 동기화합니다.
- 지정 공격 / 광역 공격 / 지속 디버프 / 환경 기술은 에너미 턴에 확정 순서대로 실행합니다. 앞 행동의 약화·기절·반격 결과를 다음 행동이 사용합니다. 격파된 에너미의 남은 행동은 건너뜁니다.
- **소환은 기존 규칙대로 암시 턴에 실행**하며 행동 1회를 사용합니다. 다른 스킬보다 뒤에 배치해도 소환 시점은 암시 턴입니다. 에너미 턴에는 중복 소환하지 않습니다.
...
각 대상마다(수비 포지션의 "보호" 대상이면 방어자로 치환):
  ...
  그 캐릭터가 이번 라운드 방어 중이면: dmg = max(0, dmg − 방어경감량)
...
`summon_count`만큼 소환수를 생성합니다. 소환수는 암시 턴에 생성되며 `summon_hp`/`summon_attack` 값을 그대로 가집니다. 공격형 소환수는 **소환된 라운드의 에너미 턴부터** 자동으로 공격합니다. 폭발·약화·강화형은 설정한 발동 턴을 따릅니다.
...
대상이 이번 라운드 방어 중이면: dmg = max(0, dmg − 방어경감량)
보호막이 있으면 min(보호막, dmg)만큼 흡수 후 차감
남은 dmg만큼 HP 감소, 0이 되면 기절
```

암시 턴에 새로 소환된 하수인을 포함해 살아있는 공격형 소환수가 그 라운드에 공격한다.
...
| 행동 | 주목도 획득량 |
| --- | --- |
| 공격/기술 사용 | `floor(실제로 들어간 피해 × (본인 포지션이 수비면 4, 아니면 1) × (1 + 존재감))`. 실제로 들어간 피해는 쇠약 증폭을 반영하고 적의 남은 체력을 넘는 몫(오버킬)은 빼며, 여러 대상을 맞히면 합산합니다 |
| 반격 피해(에너미 턴) | 공격과 같은 식. 실제로 입힌 반격 피해 기준으로 반격 태세를 건 **시전자**가 얻음(기절·퇴각한 시전자는 얻지 않음) |
| 지속 피해(행동 암시 턴, 예: 위해) | 없음(위해는 처음 공격할 때만 주목도가 쌓임) |
| 방어 | `floor((성장 등급 lv × 20) × (1 + 존재감))` |
| 치유 | `floor(실제 회복량 × 2 × (1 + 존재감))` (오버힐이 꺼져 있어 최대 체력에 막혀 실제로 덜 찬 경우, 그 실제 회복량 기준). 여러 명을 치유하는 기술은 회복받은 인원의 평균 회복량을 씁니다 |
...
- 전투 화면의 캐릭터 카드에서 이름 옆에 눈 아이콘과 함께 표시됩니다(`apps/web/app/battle/components/BattleArena.tsx`). 관리자 화면과 러너 관전 화면(`readOnly`) 모두에 보입니다.
...
| 방어력 / 방어력 증폭 / 방어 효율 | 방어 시 피해 경감량 계산에 쓰이는 3개 값 |
| 피해 증폭 | 내가 주는 피해에 곱해지는 배율 |
| 피해 감소 | 내가 받는 피해를 비율로 줄여주는 값. 기본값은 포지션(faction) 기준으로 공격/치유 30%, 수비 50%(`calculate_stat_grade_totals`) |
...
| 주목도 | 캐릭터 고정 스탯이 아니라 전투 중 행동으로 쌓이는 값(6-1절 참고). 캐릭터의 `attn` 컬럼 자체는 전투와 무관한 값이라 캐릭터 상세 정보 화면에는 노출되지 않습니다 |
